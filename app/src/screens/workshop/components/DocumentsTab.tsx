...
export default function DocumentsTab({
  ...
  const handlePreview = (doc: DocumentItem) => {
    Linking.openURL(doc.uri).catch(() =>
      Alert.alert(
        t("workshopSettingsScreen.documents.previewErrorTitle"),
        t("workshopSettingsScreen.documents.previewErrorMessage")
      )
    );
  };
            ...
            <View key={section.key} style={styles.card}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>
                  {t(`workshopSettingsScreen.documents.${section.titleKey}`)}
                </Text>
              ...
              {docs.length === 0 && (
                <Text style={styles.emptyText}>
                  {t("workshopSettingsScreen.documents.noDocuments")}
                </Text>
              )}
              ...
              >
                ...
                <Text style={styles.dashedButtonText}>
                  {isUploading
                    ? t("workshopSettingsScreen.documents.adding")
                    : canUpload
                      ? t("workshopSettingsScreen.documents.addType", {
                          type: t(
                            `workshopSettingsScreen.documents.${section.titleKey}`
                          ),
                        })
                      : t("workshopSettingsScreen.documents.limitReached")}
                </Text>
              </Pressable>
            </View>
          );
