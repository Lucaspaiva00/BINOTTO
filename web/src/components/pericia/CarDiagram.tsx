...
function VehiclePanel({ panel, state, selected, canOpen, onSelect }: { panel: Panel; state?: PartInspection; selected: boolean; canOpen: boolean; onSelect: () => void }) {
  ...
  return <mesh position={panel.position} onClick={click} onPointerEnter={(event) => { event.stopPropagation(); setHovered(true); document.body.style.cursor = canOpen ? "pointer" : "default"; }} onPointerLeave={() => { setHovered(false); document.body.style.cursor = "default"; }}>
    <boxGeometry args={panel.size} /><meshStandardMaterial color={color} metalness={.45} roughness={.35} emissive={selected || hovered ? "#ffffff" : "#000000"} emissiveIntensity={selected ? .32 : hovered ? .12 : 0} />
...
function Vehicle(props: CarDiagramProps) {
  ...
  return <group rotation={[0, -.42, 0]} scale={profile.scale}>
    ...
    {[-1, 1].flatMap((x) => [-1.3, 1.35].map((z) => <mesh key={`${x}-${z}`} position={[x, .28, z]} rotation={[Math.PI / 2, 0, 0]}><cylinderGeometry args={[.37, .37, .2, 28]} /><meshStandardMaterial color="#080b10" roughness={.48} /></mesh>))}
  </group>;
}
